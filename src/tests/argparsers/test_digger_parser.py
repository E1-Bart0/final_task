...
from src.digger import dir_path, file_path, parse_args
...
def test_dir_path__return_path():
    ...
    assert dir_path(path_to_directory) == path_to_directory
...
def test_file_path__return_path(extension):
    file = NamedTemporaryFile(suffix=extension)
    assert file_path(file.name) == file.name
...
@patch("src.digger.dir_path", side_effect=lambda x: x)
@patch("src.digger.file_path", side_effect=lambda x: x)
